...
public class KantineAanbod {
    ...
    private HashMap<String, Integer> prijzen;
    ...
    public KantineAanbod(String[] artikelnaam, int[] prijs, int[] hoeveelheid) {
        ...
        prijzen = new HashMap<String,Integer>();
        int kortingsArtikel = new Random().nextInt(artikelnaam.length);
        ...
        for(int i=0;i<artikelnaam.length;i++)
        {
            ...
            {
                if(j==kortingsArtikel){
                    double kortingPrijs = (double)prijs[i]/100 * 0.2;
                    artikelen.add(new Artikel(artikelnaam[i], prijs[i], kortingPrijs));
                } else {
    ...
    private void vulVoorraadAan(String productnaam){
    	...
    	int prijs = prijzen.get(productnaam);
